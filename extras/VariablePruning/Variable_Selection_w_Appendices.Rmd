...
So what is the posterior on the variable <code>explanatory</code> that we have been calculating?  

Roughly, by Bayes law <code>P[explanatory | data]</code> is going to be of the form <code>P[explanatory] P[data | explanatory]/Z</code>.  Under normality assumptions <code>P[data | explanatory]</code> is going to be of the form <code>c(1 - exp(-s residual_variance)/exp(-s initial_variance))</code>.  
...
In both cases (ignoring priors, and ignoring degrees of freedom) for normalized data <code>initial_variance</code> is a constant. So for a given data set both the posterior <code>P[explanatory | data]</code> and the F-test are monotone functions in <code>residual_variance</code>.  Any choice on a threshold for one of these values is approximately equivalent to a choice subject to a transformed threshold on the other.
