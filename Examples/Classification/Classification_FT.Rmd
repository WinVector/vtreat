...
Note that for the training data `d`: `fit_prepare(transform_spec, d)` is **not** the same as
...
****TODO: need the missingess imputation vignette in the new notation ****
**missingness_imputation**: The function or value that vtreat uses to impute or "fill in" missing numerical values. The default is `mean()`. To change the imputation function or use different functions/values for different columns, see the Imputation example [TODO - do I have one?]
...
**is_BAD**: Produced for numerical variables: an indicator variable that marks when the original variable was missing or `NaN`.
