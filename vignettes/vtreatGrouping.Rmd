...
showGroupingBehavior = function(groupcol, title) {
  ...
  if(have_ggplot && have_dplyr) {
    ...
    plt = ggplot(data=dtmp, aes_string(x="timeIndex", y="conc", color=groupcol)) + 
...
For this example, we will use the `Theosph` dataset: data from an experiment on the pharmacokinetics of theophylline. This appears to be a confirmation experiment, with well predicted results, and we will use it to demonstrate the desired effects of _y_-stratification while also respecting a grouping constraint. 
...
Suppose we wanted to fit a model to analyze how a patient's weight affects how theophylline is metabolized, and validate that data with three-fold cross-validation. It would be important that all readings from a given patient stay in the same fold. We might also want the population in each fold to have similar distributions of theophylline concentrations curves.

Recall that the goal of _y_-stratification is to insure that all samples from the data have as close to identical  _y_ distributions as possible. This may not be possible when we also have to obey a grouping constraint.
...
We can compare the mean theophylline concentration and the average pharmacokinetic profile for each fold, for both of the subject-preserving partitions.
...
We see that the stratification reduces some of the variation between groups.
