...
version `0.5.26` and newer can [directly model categorical outcomes "in link units" using logistic regression](https://github.com/WinVector/Examples/blob/master/PCR/YAwarePCAclassification.md)
without the 0/1 indicator encoding,
this version is currently available [here](https://github.com/WinVector/vtreat))
and mean-zero.  
...
To get this effect we must set the argument `catScaling=TRUE` in `vtreat::designTreatmentsC` or `vtreat::mkCrossFrameCExperiment`.  WE demonstrate this below.
...
```{r}
glm(y~x_lev_NA,data=dTrainCTreatedScaled,family=binomial)
glm(y~x_lev_NA,data=dTrainCTreatedScaled2,family=binomial)
```
