...
```{r plot}
vars <- treatmentsC$scoreFrame$varName[treatmentsC$scoreFrame$sig<1]
princ <- prcomp(as.matrix(dTrainCTreatedScaled[,vars]))
ndim <- 2
proj <- as.data.frame(predict(princ,newdata = 
                                as.matrix(dTrainCTreatedScaled[,vars]))[,1:ndim],
                      stringsAsFactors = FALSE)
proj$y <- dTrainCTreatedScaled$y
library('ggplot2')
ggplot(data=proj,aes(x=PC1,y=PC2,color=y)) + geom_point(size=2)
# plot should look poor, as data was just "typed in" with no strong relation
# between y and the variables.
```
