...
- `varMove` : logical TRUE if the variable varied during training; only variables that move will be in the treated frame.
...
- The scoring procedure itself involves a fit (linear regression for `PRESSRsquared` or logistic regression for `catPRSquared`).  So in each of these cases we would like the regression itself to only be evaluated on held-out data.  The PRESS statistic does just that (fast 1-way cross validation).   The `catPseudoRSquared` performs explicit 1-way cross validation for small data sets and hold-out scoring for larger data sets.
...
scales and centers all the variables to mean 0, and slope 1 with respect to y:
