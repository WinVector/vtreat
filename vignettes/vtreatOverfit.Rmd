...
Below we generate an example data frame with no relation between x and y.  We are using a synthetic data set so we know what the "right answer is" (no signal).  False fitting on no-signal variables is bad for at least two reasons:
...
There are, of course, other methods to avoid the bias introduced in using the same data to generate the variable encodings and then train a model using those variables.  vtreat incorporates a number of these including smoothing controlled through 'smFactor' and pruning of rare levels controlled through 'rareSig'.  A more effective technique: cross-constructed training frames can also be accessed by using 'mkCrossFrameCExperiment' or 'mkCrossFrameNExperiment'
